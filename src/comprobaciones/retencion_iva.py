...
def calcular_porcentaje_retencion_iva(
    ...
    # =========================================================================
    if exc is not None:
        descripciones = df_excepciones_art3.loc[df_excepciones_art3['codigo'] == exc,
        'descripcion'].values
        if len(descripciones) > 0:
            desc = descripciones[0]
        else:
            desc = f"Excepción Art.3 num.{exc}"
...
def aplicar_retencion_iva(df: pl.DataFrame) -> pl.DataFrame:
    ...
    ).with_columns(pl.lit("SERVICIO").alias("tipo_concepto_iva"))

    resultados_modificados = (
        df_bienes_a_servicios.with_columns(
            pl.struct(columnas)
            .map_elements(
                lambda row: calcular_porcentaje_retencion_iva(
                ...
                return_dtype=pl.Struct(
                    {"porcentaje": pl.Int64, "articulo": pl.Utf8, "motivo": pl.Utf8}
                ),
            )
            .alias("resultado_iva")
        )
        .with_columns(
            [
                pl.col("resultado_iva")
                .struct.field("porcentaje")
                .alias("porcentaje_retencion_iva_modificado"),
                pl.col("resultado_iva")
                .struct.field("articulo")
                .alias("articulo_iva_modificado"),
                pl.col("resultado_iva")
                .struct.field("motivo")
                .alias("motivo_iva_modificado"),
            ]
        )
        .drop("resultado_iva")
        .select(
    ...
    resultados = (
        ...
        .with_columns(
            [
                pl.col("resultado_iva")
                .struct.field("porcentaje")
                .alias("porcentaje_retencion_iva"),
    ...
    resultados_final = resultados.join(
        resultados_modificados, on="numero_ruc", how="left"
    )
