...
def calcular_retenciones(engine_data_fact: Engine) -> pl.DataFrame:
    ...
    try:
        ...
        logger.error(f"[calcular_retenciones] Paso 1 — Fallo en procesamiento(): {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[calcular_retenciones] Paso 2 — Fallo en aplicar_retencion_iva(): {e}", exc_info=True)
        raise

    try:
        aplicado_renta = aplicar_retencion_renta(base_rucs_corregida_clasificada)
    except Exception as e:
        logger.error(f"[calcular_retenciones] Paso 2 — Fallo en aplicar_retencion_renta(): {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[calcular_retenciones] Paso 3 — Fallo al unir IVA y Renta: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[calcular_retenciones] Paso 5 — Fallo al leer 'tabla_retenciones': {e}", exc_info=True)
    ...
    try:
        ...
        logger.error(
            f"[calcular_retenciones] Paso 5 — Fallo al seleccionar columnas de 'tabla_retenciones' — "
    ...
    try:
        ...
        logger.error(f"[calcular_retenciones] Paso 6 — Fallo en formatear_para_rds(): {e}", exc_info=True)
        raise
...
def formatear_para_rds(df: pl.DataFrame) -> pl.DataFrame:
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo en renombramientos: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo en derivaciones de columnas: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo al derivar cedula/cedula_str: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo en casteos a String: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo en casteos a TINYINT (0/1): {e}", exc_info=True)
        raise
    ...
    try:
        ...
        logger.error(f"[formatear_para_rds] Fallo al descartar columnas finales: {e}", exc_info=True)
        raise
